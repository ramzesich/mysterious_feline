// LEVEL 1 — Neon Outskirts. Bumbot runs right, along the rooftops of a cyberpunk city, to the
// cat feeder waiting at the far end. Registered into LEVELS by map.js.
//
// Design budget (horizontal, 700x350, moveSpeed 7 / gravity 0.8 / jumpForce 15):
...
//   * The whole level is completable without spending a single snack. Meows are a
//     convenience, never a requirement.
//   * Only ~310px of air exists above the ground and the camera never pans vertically *in this
//     level*, so nothing playable should sit above y ~265.
//
// The ground data says "rooftops" out loud: every pit is the alley between two buildings, so gaps
// only ever land on a building boundary and each stretch of ground between them is one roof with
// its own character. Read that rhythm before moving anything — a gap dropped mid-roof breaks the
// whole read.
//
...
const LEVEL_1 = {
    ...
    // which is what keeps the snack economy optional. Widths ramp up across the level.
    pits: [
        ...
        { x: 11700, width: 190 },
        { x: 12900, width: 190 },
        { x: 13950, width: 200 }, // The widest alley in the level, at the max fair width
        { x: 15250, width: 150 }
    ],
    ...
    // The level's wildlife. `axis` picks both the behaviour and the species, because up here
